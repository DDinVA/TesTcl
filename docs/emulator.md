...
current report covers 989 of 989 target F5 commands (100%). This is test-input
coverage, not a semantic-fidelity score; commands may still require deeper
semantic vectors and independent TMOS 17.5 differential observations.
...
```json
{"id":"http-host","output":{"status":"ok","tcl_return_code":0,"value":"api.example.com","event_trace":[{"sequence":0,"event":"HTTP_REQUEST","fired":true,"reason":"structured-log","source":"bigip-log","state_observed":false,"command_status":"ok","tcl_return_code":0}]}}
```
...
The external collector adds one bounded `event_trace` entry because the
correlated BIG-IP log line proves that the temporary rule reached its selected
event handler. `state_observed` remains `false`: the log does not prove the
device's internal event state or the full event ordering. Generated command
capture plans compare the event name and `fired` flag in addition to the
command result fields. Older records without `event_trace` should be
re-collected before assembling a plan generated by the current tooling.
